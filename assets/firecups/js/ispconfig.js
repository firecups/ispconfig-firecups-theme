var ISPConfig = {
	...
	onAfterContentLoad: function (url, data) {
		...
		if (ISPConfig.options.useComboBox == true) {
			setSelect2Box('.main-content', '');
			setSelect2Box('#pageContent', 'form');
		}
		...
		ISPConfig.callHook('onAfterContentLoad', {
			'url': url,
			'data': data
		});
	},
	...
	submitUploadForm: function (formname, target) {
		...
		$('input[type="file"]').closest("form").attr({
			target: frame_id,
			action: target
		}).submit();
	},
	...
	changeTab: function (tab, target, force) {
		...
		if (ISPConfig.tabChangeDiscard == 'y' && !force) {
			if ((idel.length < 1 || id) && (ISPConfig.pageFormChanged == false || window.confirm(ISPConfig.tabChangeDiscardTxt))) {
				var next_tab = tab;
				if (id) ISPConfig.loadContent(target, {
					'next_tab': next_tab,
					'id': id
				});
				else ISPConfig.loadContent(target, {
					'next_tab': next_tab
				});
			} else {
				return false;
